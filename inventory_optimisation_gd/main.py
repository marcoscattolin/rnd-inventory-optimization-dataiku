...
if __name__ == '__main__':
    ...
    with open(os.path.join('config', 'solver.yaml')) as file:
        solver_config = yaml.safe_load(file)   

    with open(os.path.join('config', 'schema.yaml')) as file:
    ...
    pipeline(
        ...
        pd.read_csv(os.path.join(path, schema_config['demad_file']['file_name'])),
        config
    )